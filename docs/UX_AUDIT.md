...
milestones. Five vertical slices were implemented:
...
2. The rooms catalog can retain and resolve exact canvas and item-definition
   versions without breaking existing `Store` adapters.
3. Client observers are centrally failure-isolated, accept `AbortSignal`
   ...
   stream.
4. `start({ until })` turns the common connection-plus-readiness sequence into
   one semantic operation while retaining the no-argument contract.
...
The pass is additive except for the intentional configuration-validation timing
and definition-mutation corrections recorded in `MAJOR_VERSION_NOTES.md`.
...
| Catalog versions | Registering v2 under an existing ID replaced v1, so a room pinned to v1 could silently become unavailable or resolve the wrong definition. | `VersionedCatalogStore` provides exact ID/version lookups. Reference stores retain all versions; the conformance kit requires two same-ID generations; legacy adapters retain their original behavior. Returned IDs and versions are checked. | `server/pkg/roomsdk/store_test.go`, `room_template_test.go`, and `roomsdktest/store.go` |
| Observer dispatch | One throwing UI callback could prevent later callbacks and escape into transport, simulation, fullscreen, overlay, or render code. Teardown required tracking many unrelated functions. | Every runtime observer uses one failure-isolated primitive. Each subscription still returns an idempotent unsubscribe and additionally accepts `{ signal }`. Intent and participant-projection failures use safe fallbacks and become recoverable `consumer_callback_failed` events. Same-function deduplication is preserved. | `room-observers.test.ts`, `observers.test.ts`, `participant-roster.test.ts`, `runtime-lifecycle.test.ts`, fullscreen, overlay, and driver tests |
...
| Startup readiness | The common safe reveal required `await start(); await whenStartupReady();`, while `start()` alone meant only transport-connected/JOIN-sent. | `start({ until: "initialized" | "presented" })` composes the documented gates. `start()` remains connection-only and returns the same cached promise; invalid JavaScript boundary strings fail before connection. | `runtime-lifecycle.test.ts` and `asset-pipeline.test.ts` prove both façade levels and typo handling. Examples use the one-call form. |
...
An existing store adapter remains valid:

```go
type Store interface {
    LoadCanvas(context.Context, string) (CanvasRecord, error)
    LoadItemDefinition(context.Context, string) (ItemDefinitionRecord, error)
    // snapshot methods omitted
}
```

A store that supports rolling versions adds the optional capability:

```go
type VersionedCatalogStore interface {
    LoadCanvasVersion(context.Context, string, uint32) (CanvasRecord, error)
    LoadItemDefinitionVersion(context.Context, string, uint32) (ItemDefinitionRecord, error)
}
```

The SDK capability-checks this interface and requests the room template's exact
version. It does not make every external store change atomically with the SDK.
...
| Catalog lookup was ID-only | Implemented compatibly | Exact lookup is optional so deployed stores can migrate on their schedule. |
...
| Startup required callers to understand internal gates | Implemented compatibly | The semantic overload removes ceremony without reinterpreting legacy `start()`. |
...
| Public declaration fingerprints expose release-test internals | Retained | They are repository release fixtures, not package exports. The 0.3.0 baseline must be regenerated only from clean declarations. |
...
- Existing `Store` implementations compile and run unchanged. They keep
  latest-by-ID behavior until they opt into exact version lookup.
- Existing no-argument `RoomSession.start()` and `CanvasRuntime.start()` keep
  their connection-only timing and cached-promise behavior.
